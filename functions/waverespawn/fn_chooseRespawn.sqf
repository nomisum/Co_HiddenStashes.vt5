...
if (leader _group != player) exitWith {
    [side player,"Teamlead is choosing respawn."] call wita_waverespawn_fnc_respawnHint;
};
...
if (side player == EAST) then {[true] call wita_sectors_fnc_drawSectors};
openMap [true, true];
...
mcd_onRespawnKeyDown = (findDisplay 46) displayAddEventHandler ["KeyUp", {
    ...
    _condition = if (side player == WEST) then {
    ...
    if (_condition) then {
        if (side player == WEST) then {["PLEASE SELECT A COMMANDVEHICLE"] call wita_ui_fnc_confirmHint} else {["PLEASE SELECT A SECTOR"] call wita_ui_fnc_confirmHint};
        ...
        [false] call wita_ui_fnc_twoLineHint;
        _pos = if (side player == WEST) then {getPos wita_selectedRespawnObject} else {wita_selectedRespawnPos};
