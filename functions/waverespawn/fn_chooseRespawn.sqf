...
if (side _group == INDEPENDENT && count (missionNamespace getVariable ["wita_cv_allCVs",[]]) < 2) exitWith {[_group,getPos (wita_cv_allCVs select 0)] remoteExec ["wita_waverespawn_fnc_respawnGroup",2,false]};
...
_zoom = if (side _group == INDEPENDENT) then {0.12} else {0.02};
    ...
    "onMapSingleClick",
    {
        if (PLAYERSIDE == INDEPENDENT) then {
            ...
            if (_respawnObject getVariable ["wita_respawnObject_isActive", false]) then {
                [INDEPENDENT,_respawnObject] call wita_waverespawn_fnc_respawnSelectorMarker;
            };
        };
...
mcd_onRespawnKeyDown = (findDisplay 46) displayAddEventHandler ["KeyUp", {
    ...
    _condition = if (PLAYERSIDE == INDEPENDENT) then {
    ...
    if (_condition) then {
        if (PLAYERSIDE == INDEPENDENT) then {["PLEASE SELECT A COMMANDVEHICLE"] call wita_ui_fnc_confirmHint} else {["PLEASE SELECT A SECTOR"] call wita_ui_fnc_confirmHint};
        ...
        [false] call wita_ui_fnc_twoLineHint;
        _pos = if (PLAYERSIDE == INDEPENDENT) then {getPos wita_selectedRespawnObject} else {wita_selectedRespawnPos};
