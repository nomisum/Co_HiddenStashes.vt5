...
if (_on) then {
    ...
    _trg setTriggerStatements ["ace_player getVariable ['cbrn_autoDamage', false] && { !(ace_player getVariable ['cbrn_stoppedAutoDamage', false]) && {ace_player in thisList}}", "ace_player setVariable ['cbrn_stoppedAutoDamage', true];  ace_player setVariable ['cbrn_damage', 0]; 'Success!' hintC 'The contamination stopped, it should have not come to this in the first place!'", ""];
